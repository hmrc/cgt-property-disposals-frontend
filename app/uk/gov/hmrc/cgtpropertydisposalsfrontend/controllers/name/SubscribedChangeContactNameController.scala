...
class SubscribedChangeContactNameController @Inject()(
  ...
  override def updateContactName(journey: Subscribed, contactName: ContactName): JourneyStatus =
    journey.copy(subscribedDetails = journey.subscribedDetails.copy(contactName = contactName))

  override def contactName(journey: Subscribed): Option[ContactName] = None
