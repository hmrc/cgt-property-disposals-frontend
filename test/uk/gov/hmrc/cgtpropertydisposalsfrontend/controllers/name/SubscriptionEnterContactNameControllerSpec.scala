...
class SubscriptionEnterContactNameControllerSpec
  ...
  override val controller: SubscriptionEnterContactNameController = instanceOf[SubscriptionEnterContactNameController]

  implicit lazy val messagesApi: MessagesApi = controller.messagesApi

  override val validJourney: SubscriptionReady = sample[SubscriptionReady]

  override def updateContactName(contactName: ContactName, journey: SubscriptionReady): JourneyStatus =
  ...
  "SubscriptionEnterContactNameController" when {
    ...
    "handling submitted names" must {
      behave like enterContactNamePage(
        data => controller.enterContactName()(FakeRequest().withFormUrlEncodedBody(data: _*).withCSRFToken)
      )
    }
