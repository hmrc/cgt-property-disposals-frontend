...
class InsufficientConfidenceLevelControllerSpec
  ...
  def mockGetNumberOfUnsuccessfulAttempts(
    ggCredId: GGCredId
  )(result: Either[NameMatchError[IndividualNameMatchDetails], Option[UnsuccessfulNameMatchAttempts[IndividualNameMatchDetails]]]) =
    (mockBprNameMatchService
      .getNumberOfUnsuccessfulAttempts[IndividualNameMatchDetails](_: GGCredId)(_: Reads[IndividualNameMatchDetails], _: ExecutionContext))
      .expects(ggCredId, *, *)
        ...
        "a valid SA UTR and name are entered but they cannot be matched to a BPR and the " +
          ...
          inSequence {
            ...
            mockAttemptNameMatch(validSautr, validName, ggCredId, Some(previousUnsuccessfulNameMatchAttempt))(
              Left(NameMatchError.NameMatchFailed(UnsuccessfulNameMatchAttempts(2, 3, IndividualNameMatchDetails(validName, validSautr))))
            )
          }
      ...
      "display an error page" when {
        ...
        "there is an error storing a retrieved BPR in mongo" in {
          inSequence {
            ...
            mockStoreSession(session(SubscriptionMissingData(bpr, None, ggCredId)))(Future.successful(Left(Error(""))))
          }
      ...
      "update the session and redirect to the start endpoint" when {
        ...
        "the user submits a valid SA UTR and name and a BPR can be retrieved" in {
          inSequence {
            ...
            mockStoreSession(session(SubscriptionMissingData(bpr, None, ggCredId)))(Future.successful(Right(())))
          }
