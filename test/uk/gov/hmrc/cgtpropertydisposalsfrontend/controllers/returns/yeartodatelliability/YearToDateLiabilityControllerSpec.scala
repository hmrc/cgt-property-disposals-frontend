...
import uk.gov.hmrc.cgtpropertydisposalsfrontend.models.upscan.{UploadReference, UpscanFailure, UpscanSuccess, UpscanUpload}
...
class YearToDateLiabilityControllerSpec
  ...
  def redirectToStartWhenFurtherReturnIneligibleForCalculation(performAction: () => Future[Result]): Unit = {
    val (session, fillingOutReturn, _) =
      sessionWithSingleDisposalState(
        None,
        Some(sample[DisposalDate]),
