...
class InitialGainOrLossControllerSpec
  ...
  "InitialGainOrLossController" when {
    ...
    "initialGainOrLoss is not present in draftReturn" should {
      ...
      "display the page with backlink to tasklist" in {
    ...
    "initialGainOrLoss is present in returnDraft" should {
      ...
      "display the page with backlink to check your answers" in {
    ...
    "there is an error updating the draft return" should {
      ...
      "there is an error updating the session" should {
        "show a technical error page" in {
          inSequence {
            mockAuthWithNoRetrievals()
            mockGetSession(session)
    ...
    "submitting initial gain or loss" should {
      def performAction(data: (String, String)*): Future[Result] =
        controller.submitInitialGainOrLoss()(FakeRequest().withFormUrlEncodedBody(data: _*))
      ...
      "show required error" when {
        ...
        "show the amount of gain is invalid" in {
