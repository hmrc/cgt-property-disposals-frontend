    ...
    "display the agents landing page" in {
      ...
      checkPageIsDisplayed(
        ...
        messageFromMessageKey("agentsLandingPage.title"), doc => {
          doc.select(".button").attr("href") shouldBe s"${viewConfig.agentsSignInUrl}"
          doc.select("#nonResidentsRebasingUrl > a").attr("href") shouldBe s"${viewConfig.nonResidentsRebasingUrl}"
          doc.select("#createAgentsAccountUrl > a").attr("href") shouldBe s"${viewConfig.createAgentsAccountUrl}"
          doc.select("#nrcgtReturn-1 > a").attr("href") shouldBe s"${viewConfig.nrcgtReturn}"
          doc.select("#nrcgtReturn-2 > a").attr("href") shouldBe s"${viewConfig.nrcgtReturn}"
          doc.select("#contact-hmrc-1 > a").attr("href") shouldBe s"${viewConfig.contactHmrc}"
          doc.select("#contact-hmrc-2 > a").attr("href") shouldBe s"${viewConfig.contactHmrc}"
          doc.select("#cgtUrl > a").attr("href") shouldBe s"${viewConfig.cgtUrl}"
        }
      )
    }
